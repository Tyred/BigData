...
##### Note: For multiple datasets, use the name of the first that appears in the table.
...
For this experiment, we use queries to perform the similarity search evaluation.
...
``` 
This code uses the models that you have trained. Either way, save the coded data in ./data/<experiment\>/coded_data folder.
...
```bash
python3 pdist.py -d <dataset_name> -m <model_name>
```

The dataset name is the dataset that we use for evaluation and the model_name is the name of dataset used for training, in case of multiple datasets for training, the model_name is the name of the first one. Example below runs the evaluation of the AP2 experiment.
